...
当前支持以下 7 个子场景，生成时需覆盖这些场景的典型交互。**后续新增场景必须在 `data/scenarios/` 中注册**：
...
2. **中文人物数据支持**
   - 将 PERSONA_FROM_MSC_PROMPT 改为中文提示词
   - 为中文提示词添加输出实例
   - 将事件图生成示例数据改为中文

3. **对话模式改为单人和AI助手对话**
   - agent_a 固定为家庭智能助手角色，具备设备控制能力
   - agent_b 从数据集采样真实人物特征
   - AI助手描述包含智能灯、空调、窗帘、门锁、摄像头等设备操作能力

4. **场景化时间范围配置**
   - 为7个场景添加对应的时间段配置（上班离家6-9点、小孩放学15-18点等）
   - 修改 `get_random_time()` 函数支持场景化时间生成
   - 支持跨天时间范围（如异常检测场景22:00-04:00）

5. **家庭设备库创建**
   - 创建 `/locomo/data/devices/home_devices.json` 设备配置文件
   - 包含4大类12种设备：感知类、环境感知、执行设备、交互设备
   - 每个设备包含场景关联、典型事件、操作概率、状态转移、数据类型等字段
   - 建立场景-设备映射关系
