...
## 重要约束
- devices 至少包含当前 event.object_id 以及当前情景描述中提到的关键设备
- 每个设备 ID 必须来自可控设备列表或房间与设备布局
...
def validate_llm_devices_result(result, available_devices, required_device_id):
    if not isinstance(result, dict):
    ...
    if required_device_id not in devices:
        raise ValueError(f"devices missing current event object_id: {required_device_id}")
    return devices
...
def generate_split_annotated_event_llm(context, run_json_trials_func, previous_events):
    ...
    devices_prompt = LLM_EVENT_DEVICES_PROMPT.format(
        scenario=scenario,
        episode_date=episode_date.strftime('%Y-%m-%d'),
        timestamp=timestamp,
        room_device_layout=context['room_device_layout'],
    ...
    devices_result = run_json_trials_func(
        devices_prompt,
        num_gen=1,
        num_tokens_request=900,
        temperature=0.6,
    )
    devices = validate_llm_devices_result(devices_result, available_devices, event['object_id'])
